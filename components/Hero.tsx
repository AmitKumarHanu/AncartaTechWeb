  ...
  return (
    <section className="relative bg-gradient-to-br from-blue-700 via-blue-800 to-blue-900 text-white overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute bottom-0 left-1/2 w-72 h-72 bg-sky-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-4000"></div>
      </div>
        ...
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold mb-6 leading-tight">
            {title.split(' ').map((word, index) => {
              if (word === 'Digital' || word === 'Smart' || word === 'Solutions' || word === 'Ancarta') {
                return (
                  <span key={index} className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-blue-300">
                    {word}{' '}
                  </span>
                );
              }
          ...
          {showCTA && (
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
              ...
              <Button href={secondaryCTALink} variant="outline" className="border-white text-white hover:bg-white hover:text-blue-700">
                {secondaryCTAText}
              </Button>
            </div>
          )}
